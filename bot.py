...
async def style(interaction, name: str):
...
async def mode(interaction, name: str):
    await interaction.response.send_message(MODE_TRELLO[name])

@tree.command(name="psyches")
@app_commands.choices(rarity=[app_commands.Choice(name=k.title(), value=k) for k in PSYCHES_TRELLO])
async def psyches(interaction, rarity):
    await interaction.response.send_message(PSYCHES_TRELLO[rarity.value])

@tree.command(name="clan")
@app_commands.choices(rarity=[app_commands.Choice(name=k.title(), value=k) for k in CLAN_TRELLO])
async def clan(interaction, rarity):
